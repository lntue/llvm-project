...
function(_get_compile_options_from_flags output_var)
  ...
  check_flag(ADD_PUBLIC_PACKAGING_FLAG ${PUBLIC_PACKAGING_FLAG} ${flags})

  if(LLVM_COMPILER_IS_GCC_COMPATIBLE)
    if(ADD_PUBLIC_PACKAGING_FLAG)
      list(APPEND compile_options "-DLIBC_COPT_PUBLIC_PACKAGING")
    endif()
    if(ADD_FMA_FLAG)
...
function(_build_gpu_object_for_single_arch fq_target_name gpu_arch)
  ...
  if("${gpu_arch}" IN_LIST all_amdgpu_architectures)
    list(APPEND compile_options "--target=${AMDGPU_TARGET_TRIPLE}")
    list(APPEND compile_options "-mcpu=${gpu_arch} -flto")
    list(APPEND compile_options "SHELL:-Xclang -mcode-object-version=none")
  elseif("${gpu_arch}" IN_LIST all_nvptx_architectures)
    list(APPEND compile_options "--target=${NVPTX_TARGET_TRIPLE}")
  ...
  # Always enable LIBC_COPT_PUBLIC_PACKAGING for GPU.
  target_compile_definitions(${fq_target_name} PUBLIC LIBC_COPT_PUBLIC_PACKAGING) 
...
# Builds the object bundle target for the GPU.
...
function(_build_gpu_object_bundle fq_target_name)
  ...
  add_dependencies(${fq_target_name}
                   ${ADD_GPU_OBJ_DEPENDS} ${packaged_gpu_names} ${stub_target_name})
endfunction(_build_gpu_object_bundle)
...
function(create_object_library fq_target_name)
  ...
  check_flag(public_packaging ${PUBLIC_PACKAGING_FLAG} ${ADD_OBJECT_FLAGS})
  ...
  if(LIBC_TARGET_ARCHITECTURE_IS_GPU)
    if(${public_packaging} AND NOT ${ADD_OBJECT_NO_GPU_BUNDLE})
      _build_gpu_object_bundle(
        ${fq_target_name}
...
function(create_entrypoint_object fq_target_name)
  ...
  check_flag(public_packaging ${PUBLIC_PACKAGING_FLAG} ${ADD_ENTRYPOINT_OBJ_FLAGS})
  ...
  if(LLVM_LIBC_ENABLE_LINTING AND NOT public_packaging)
    # Add linting to internal target.
    ...
    add_custom_target(
      ...
      DEPENDS ${fq_target_name} ${ADD_ENTRYPOINT_OBJ_SRCS}
...
function(add_entrypoint_object target_name)
  ...
  # Always generate both public packaging and internal targets for entrypoint
  # objects.
  add_target_with_flags(
    ${target_name}
    NAME ${ADD_ENTRYPOINT_OBJ_NAME}
    ADD_FLAGS ${PUBLIC_PACKAGING_FLAG}
